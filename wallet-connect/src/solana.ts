...
export async function connectSolanaWallet(
    ...
    try {
        ...
        if (!adapter.readyState || adapter.readyState === 'Unsupported') {
            throw new Error('Wallet extension not installed or not supported')
        }

        if (adapter.readyState === 'NotDetected') {
            throw new Error('Wallet extension not detected. Please install the wallet extension and refresh the page.')
        }

        if (adapter.readyState === 'Loadable') {
            // Wait a bit for the wallet to become ready
        ...
        if (error instanceof Error) {
            ...
            if (error.message.includes('WalletNotReadyError')) {
                throw new Error('Wallet is not ready. Please ensure the wallet extension is installed and unlocked, then refresh the page.')
            }
        }
